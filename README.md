...
- `src/video_intelligence_agent/ingestion/`
  Video source adapters and file-based ingestion.
- `src/video_intelligence_agent/preprocessing/`
  Motion filtering and other early-stage transforms before heavy inference.
...
- `src/video_intelligence_agent/event_engine/`
  Rule-based event logic for entering, exiting, loitering, and repeated presence.
...
- `models/`
  Model assets, checkpoints, and future exported weights.
- `logs/`
  Deployment-oriented runtime log location.
- `docs/`
...
- events are extracted and logged with clips and JSON evidence
...
- explicit ingestion, preprocessing, tracking, and event-engine modules
...
### Why Not Rely Only On Gemini?

Sending raw video or loosely sampled frames directly to Gemini would make the system more expensive, harder to debug, and less consistent. The CV pipeline creates structured evidence first, which means the LLM reasons over events instead of trying to infer everything from raw media.

### How Does The Pipeline Scale?

The system is modular and stage-based, so scaling mostly means replacing the ingestion layer with stream readers, running per-camera workers, pushing events into a database or queue, and keeping the LLM in the final reasoning stage instead of on the critical path for every frame.

### How Do You Detect Events?

Events are generated by a rule engine over track state. Entry and exit are inferred from movement between edge and center zones, loitering is based on dwell time and low spatial drift, and repeated presence can be added by counting recurring track/identity appearances over time windows.
